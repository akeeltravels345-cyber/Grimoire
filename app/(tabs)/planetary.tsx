...
export default function PlanetaryScreen() {
  ...
  const getDateForDay = useCallback((dayIndex: number) => {
    ...
    // Show current week: past days are this week, future days are this week
    if (diff > 3) diff -= 7;
    if (diff < -3) diff += 7;
  ...
  useEffect(() => {
    ...
    // For the selected day, calculate hours with current time awareness only if it is today
    const allHours = getPlanetaryHours(targetDate);

    // If not today, clear isCurrent from all hours
    if (!isToday) {
      allHours.forEach(h => { h.isCurrent = false; });
    }

    setHours(allHours);
  }, [selectedDay, isToday, getDateForDay]);

  // Re-check current hour every minute (only matters for today)
  useEffect(() => {
    if (!isToday) return;
    const interval = setInterval(() => {
      const allHours = getPlanetaryHours();
      setHours(allHours);
    }, 60000);
  ...
  const handleWeekDayTap = (dayIndex: number) => {
    setSelectedDay(dayIndex);
    setHourTab('day');
    Haptics.selectionAsync();
  };

  const selectedDateStr = getDateForDay(selectedDay).toLocaleDateString('en-US', {
  ...
  return (
    <SafeAreaView edges={['top']} style={styles.container}>
      <View style={styles.topbar}>
        <View>
          <Text style={styles.topbarTitle}>Planetary Magic</Text>
          <Text style={styles.topbarSub}>{selectedPlanet.day} — Day of {selectedPlanet.name}</Text>
        </View>
      </View>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: insets.bottom + 16 }} showsVerticalScrollIndicator={false}>
        ...
        {/* Weekly Grid — Selection Controls */}
          ...
          {dayAbbrevs.map((abbr, i) => {
            ...
            return (
              <Pressable
                ...
                style={[
                  styles.weekCell,
                  isSelected && { borderColor: planet.color, backgroundColor: planet.color + '15' },
                  !isSelected && isTodayCell && { borderColor: theme.primary + '40' },
                ]}
                onPress={() => handleWeekDayTap(i)}
              >
        ...
        {!isToday ? (
          <View style={styles.dateIndicator}>
            ...
            <Text style={styles.dateIndicatorText}>{selectedDateStr}</Text>
            <Pressable onPress={() => { setSelectedDay(todayDow); Haptics.selectionAsync(); }}>
        ...
        <View style={[styles.dayCard, { borderColor: selectedPlanet.color + '40' }]}>
          <View style={styles.dayCardHeader}>
            <Text style={styles.dayEmoji}>{selectedPlanet.emoji}</Text>
            <View style={{ flex: 1 }}>
              <Text style={styles.dayLabel}>
                {isToday ? "TODAY'S RULING PLANET" : `${selectedPlanet.day.toUpperCase()} RULER`}
              </Text>
              <View style={styles.dayNameRow}>
                <Text style={[styles.daySymbol, { color: selectedPlanet.color }]}>{selectedPlanet.symbol}</Text>
                <Text style={styles.dayName}>{selectedPlanet.name}</Text>
              </View>
            </View>
          </View>
          ...
          {/* Best Workings */}
          <Text style={styles.detailLabel}>BEST WORKINGS</Text>
          <View style={styles.chipRow}>
            {selectedPlanet.bestWorkings.map(w => (
              <View key={w} style={[styles.detailChip, { backgroundColor: selectedPlanet.color + '18' }]}>
                <Text style={[styles.detailChipText, { color: selectedPlanet.color }]}>{w}</Text>
              </View>
            ))}
          </View>

          {/* Herbs */}
          <Text style={styles.detailLabel}>HERBS</Text>
          <View style={styles.chipRow}>
            {selectedPlanet.herbs.map(h => (
              <View key={h} style={styles.subtleChip}>
                <Text style={styles.subtleChipText}>{h}</Text>
              </View>
            ))}
          </View>
          ...
          <View style={styles.chipRow}>
            {selectedPlanet.crystals.map(c => (
              <View key={c} style={styles.subtleChip}>
                <Text style={styles.subtleChipText}>{c}</Text>
              </View>
            ))}
          </View>
          ...
          <View style={styles.chipRow}>
            {selectedPlanet.colors.map(c => (
              <View key={c} style={[styles.detailChip, { backgroundColor: selectedPlanet.color + '18' }]}>
                <Text style={[styles.detailChipText, { color: selectedPlanet.color }]}>{c}</Text>
              </View>
            ))}
          </View>
        </View>
        ...
        <View style={styles.sectionHead}>
          <Text style={styles.sectionTitle}>
            {isToday ? 'Planetary Hours' : `${selectedPlanet.day} Hours`}
          </Text>
          <Text style={styles.timezoneText}>{timezone}</Text>
        </View>

        {/* Day/Night Toggle */}
        <View style={styles.phTabs}>
          <Pressable style={[styles.phTab, hourTab === 'day' && styles.phTabActive]} onPress={() => setHourTab('day')}>
            <Text style={[styles.phTabText, hourTab === 'day' && styles.phTabTextActive]}>☀️ Day Hours</Text>
          </Pressable>
          <Pressable style={[styles.phTab, hourTab === 'night' && styles.phTabActive]} onPress={() => setHourTab('night')}>
            <Text style={[styles.phTabText, hourTab === 'night' && styles.phTabTextActive]}>🌙 Night Hours</Text>
          </Pressable>
        </View>
        ...
        <View style={styles.hoursCard}>
          ...
          <View style={styles.tableHeaderRow}>
            ...
            <Text style={[styles.tableHeaderCell, { flex: 1 }]}>Planet</Text>
            <Text style={[styles.tableHeaderCell, { flex: 1.2 }]}>Energy</Text>
            <Text style={[styles.tableHeaderCell, { width: 68, textAlign: 'right' }]}>Start</Text>
          </View>

          {filteredHours.map((h) => {
            const isActive = h.isCurrent;
            return (
              <View key={`${h.type}-${h.hourNumber}`} style={[styles.tableRow, isActive && styles.tableRowActive]}>
                <View style={[styles.hourNumBox, isActive ? { backgroundColor: h.planet.color + '25' } : null]}>
                  <Text style={[styles.hourNum, isActive ? { color: h.planet.color } : null]}>{h.hourNumber}</Text>
                </View>
                <Text style={styles.hourEmoji}>{h.planet.emoji}</Text>
                <View style={{ flex: 1 }}>
                  <Text style={[styles.hourPlanetName, isActive ? { color: h.planet.color, fontWeight: '700' } : null]}>
                    {h.planet.name}
                  </Text>
                </View>
                <Text style={[styles.hourEnergySummary, isActive ? { color: h.planet.color } : null]} numberOfLines={1}>
                  {h.planet.bestWorkings[0]}
                </Text>
                <View style={{ width: 68, alignItems: 'flex-end' }}>
                  {isActive ? (
                    <View style={[styles.nowBadge, { backgroundColor: h.planet.color + '25' }]}>
                      <Text style={[styles.nowBadgeText, { color: h.planet.color }]}>NOW</Text>
                    </View>
                  ) : (
                    <Text style={styles.hourTime}>{formatHourTime(h.startTime)}</Text>
                  )}
                </View>
              </View>
            );
...
const styles = StyleSheet.create({
  ...
  // Day Card
  dayCard: { backgroundColor: theme.surface, borderRadius: theme.radius.lg, padding: 20, marginBottom: 20, borderWidth: 1, ...theme.shadows.card },
  ...
  // Hours Table
  hoursCard: { backgroundColor: theme.surface, borderRadius: theme.radius.lg, padding: 10, ...theme.shadows.card, marginBottom: 20 },
